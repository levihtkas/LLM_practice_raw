...
class FronteirAgent(Agent):
    name = "Fronteir Agent"
    color = Agent.BLUE

    def __init__(self,collection):
        self.log("Initializing Fronteir Agent")
        self.client = OpenAI()
        self.MODEL="gpt-5.1"
        self.log("contecting OAI")
    ...
    def find_similarities(self,description:str):
        ...
        self.log("Fronteir Agents has found similar products")
    ...
    def price(self,description)->float:
        document,price = self.find_similarities(description)
        self.log(
            f"Frontier Agent is about to call {self.MODEL} with context including 5 similar products"
        )
        response = self.client.chat.completions.create(
            model=self.MODEL,
            messages = self.messages_for(description,document,price),
            seed=42,
