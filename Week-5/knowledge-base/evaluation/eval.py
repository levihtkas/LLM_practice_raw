...
from implementation.answer import answer_question, db_name,fetch_document
...
def calculate_mrr(keyword:str, reterieved_docs:list):
  keyword_lower = keyword.lower()
  for rank,doc in enumerate(reterieved_docs,start=1):
...
def calculate_ndcg(keyword:str,reterieved_docs:list,k:int):
  keyword_lower = keyword.lower()

  relevances = [1 if keyword_lower in doc.page_content.lower() else 0 for doc in reterieved_docs[:k]]

  dcg = calculate_dcg(relevances,k);

  releavance = sorted(relevances,reverse=True)
  idcg = calculate_dcg(releavance,k)

  return dcg/idcg if idcg > 0 else 0.0

def evaluate_reterival(test:TestQuestion,k=10):
  reterieved_docs = fetch_document(test.question)
  mrr_scores = [calculate_mrr(keyword,reterieved_docs) for keyword in test.keywords]
  avg_mrr = sum(mrr_scores)/len(mrr_scores) if mrr_scores else 0

  ndcg_scores = [calculate_ndcg(keyword,reterieved_docs,k) for keyword in test.keywords]
...
def evaluate_all_reterival():
  """ Evaluate all reterival tests"""

  tests = load_tests()
  total_tests = len(tests)

  for index,test in enumerate(test):
    result = evaluate_reterival(test)
...
def run_cli_evaluation(test_number):
  ...
  retrieval_result = evaluate_reterival(test)
